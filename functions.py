...
def scanLine(vertices):
    ...
    lines_table = []
    lowest_y, highest_y = vertices[0][0][1],0
    lowest_x, highest_x = vertices[0][0][0],0
    ...
    for vertice in vertices:
        ...
        m = option.m([y_min,x_y_min], [y_max,x_y_max])
        if m == 0: m1 = 0
        else: m1 = 1/m
        ...
        if lowest_x > x_y_min:
            lowest_x = x_y_min
        if highest_x < x_y_max:
            highest_x = x_y_max

    result = []
    for y in range(lowest_y+1, highest_y):
        intersec = []
        aux = []
        ...
        for line in lines_table:
            ...
            if line[0] <= y and line[1] >= y:
                print(line[0],y,line[1])
                x = math.floor(line[3]*(y-line[0])+line[2])
